...
- Defines the `UnaryActivation` dataclass pairing forward and derivative logic.
...
- Provides the `LossFunction` wrapper so losses expose consistent forward/derivative signatures.
...
- Design encourages future optimizers to adhere to the `Optimizer` protocol.  
...
- Encapsulates weights/biases with gradient buffers and exposes He initialization.
...
- Caches inputs and activations for gradient computation during backprop.  
...
- Default activation is softmax, suitable for classification problems.  
